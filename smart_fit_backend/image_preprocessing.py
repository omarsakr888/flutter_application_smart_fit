...
def preprocess_for_easyocr(image_bytes: bytes) -> PreprocessedImage:
    """Lightweight EasyOCR preprocessing — resize, CLAHE, and gentle sharpen only.

    Skips border removal, deskew, and shadow reduction for speed. Use
    ``preprocess_image`` when image quality is poor or the photo is skewed.
    """
    array = np.frombuffer(image_bytes, dtype=np.uint8)
    image = cv2.imdecode(array, cv2.IMREAD_COLOR)
    if image is None:
